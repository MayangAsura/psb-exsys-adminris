...
const QuestionForm = ({ 
  ...
  const getCategories = async () => {
    ...
    if (question_bank_categories && question_bank_categories.length > 0) {
      setCategories(question_bank_categories.map(value => ({
        value: value.id,
        name: value.name,
      })));
    }
  ...
  const getQuestionBanks = async () => {
    ...
    if (question_banks && question_banks.length > 0) {
      setQuestionBanks(question_banks.map(value => ({
        value: value.id,
  ...
  // Function to submit to Supabase
  const submitToSupabase = async (submissionData) => {
    ...
    try {
      ...
      const supabaseData = {
        ...
        score: parseFloat(submissionData.score),
        bank_code: submissionData.bank_code,
        category_id: submissionData.category_id,
        parent_id: submissionData.parent_id || null,
        updated_at: new Date().toISOString()
      };

      let result;

      if (isEditing && submissionData.id) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              <option value="">Tidak ada induk soal</option>
              {parentOptions.map(question => (
                <option key={question.value} value={question.value}>
                  {question.name} - {question.question?.substring(0, 30)}...
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end space-x-3 pt-4">
            ...
            >
              {isSubmitting ? 'Menyimpan...' : (isEditing ? 'Update Soal' : 'Simpan Soal')}
            </button>
          </div>
        </div>
      </form>
