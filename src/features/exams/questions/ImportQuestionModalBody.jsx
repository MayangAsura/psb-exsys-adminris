...
function ImportQuestionModalBody({ closeModal, extraObject }) {
    ...
    const processImportedData = (rawData) => {
        ...
        if (!Array.isArray(rawData) || rawData.length === 0) {
            console.error('Invalid data format: expected array but got', typeof rawData)
        ...
        rawData.forEach((item, idx) => {
            ...
            if (isNaN(optionAnswer) || optionAnswer < 1 || optionAnswer > 5) {
                invalidEntries.push({
                    index: idx,
                    data: item,
                    reason: "OPTION_ANSWER must be a number between 1-5"
                })
                return
            }
            ...
            if (item.TYPE === 'MC' && !hasOptions) {
                invalidEntries.push({
                    index: idx,
                    data: item,
                    reason: "Multiple choice questions require at least one option"
                })
                return
            }

            // Validate that the selected answer exists in options
            if (item.TYPE === 'MC' && !answer) {
                invalidEntries.push({
                    index: idx,
                    data: item,
                    reason: "Selected answer not found in options"
                })
                return
            }
            ...
            if (isNaN(score) || score < 0) {
                invalidEntries.push({
                    index: idx,
                    data: item,
                    reason: "SCORE must be a positive number"
                })
                return
            }
            ...
            processedData.push({
                ...
                bank_code: item.BANK_CODE.toString().trim(),
                score: score,
                question_type: item.TYPE.toString().trim(),
                answer: answer,
                exam_test_id: index,
                options: options
            })
    ...
    const saveImport = async (importedData) => {
        console.log('Starting import with data:', importedData)
        ...
        try {
            ...
            console.log(`Processing ${processedData.length} valid questions`)
            ...
            for (let i = 0; i < processedData.length; i++) {
                ...
                console.log(`Processing question ${i + 1}/${processedData.length}:`, questionData)
                
                try {
                    await addQuestionMutation({ questions: questionData })
                } catch (error) {
    ...
    const handleImport = (data) => {
        console.log('Data received from FileUploads:', data)
        ...
        if (data && Array.isArray(data) && data.length > 0) {
            ...
            setDataImport([])
            dispatch(showNotification({ 
                message: "Data yang diupload tidak valid", 
                status: 0 
            }))
        }
    }
                ...
                <button 
                    className={`btn px-6 ${
                        isLoading 
                            ...
                            : success 
                            ? 'btn-success' 
                            : 'bg-green-700 text-gray-300 hover:bg-green-600 hover:text-gray-500'
                    }`}
                    onClick={() => success ? closeModal() : saveImport(dataImport)}
