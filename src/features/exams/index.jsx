...
function Exams(){
    ...
    const [originalExamData, setOriginalExamData] = useState([])
    ...
    const getExamData = async() => {
        ...
        if(!error){
            ...
            setOriginalExamData(exam_tests)
        }
    }

    const removeFilter = () => {
        setExamData(originalExamData)
    }

    const applyFilter = (params) => {
        let filteredTransactions = originalExamData.filter((t) => {return t.scheme == params.toLowerCase()})
        setExamData(filteredTransactions)
    }
    ...
    const applySearch = (value) => {
        if(value === "" || value == null) {
            ...
            let filteredTransactions = originalExamData.filter((t) => {
                return t.name.toLowerCase().includes(value.toLowerCase()) || 
                       t.exam_schedule_tests[0]?.exam_schedules?.name.toLowerCase().includes(value.toLowerCase()) || 
                       t.location.toLowerCase().includes(value.toLowerCase()) || 
                       t.room.toLowerCase().includes(value.toLowerCase())
            })
            setExamData(filteredTransactions)
        }
    }
    ...
    const deleteCurrentData = async (index) => {
        ...
        dispatch(openModal({
            ...
            extraObject : { 
                message : `Apakah Anda yakin menghapus ujian ini?`, 
                            ...
                            {examData.length === 0 ? (
                                <tr>
                                    <td colSpan="7" className="text-center py-4 text-gray-500">
                                        {originalExamData.length === 0 ? "Tidak ada data ujian" : "Tidak ada hasil pencarian"}
                                    </td>
                                </tr>
                            ) : (
                                examData.map((l, k) => {
                                    return(
                                        <tr key={k}>
                                            ...
                                            <td>
                                                <div className={`badge-primary ${l.scheme === 'Online' ? 'bg-green-400' : 'bg-orange-400'} font-semibold text-gray-50 rounded-2xl w-16 py-1 px-2`}>
                                                    {l.scheme}
                                                </div>
                                            </td>
                                            <td>{l.exam_schedule_tests[0]?.exam_schedules?.name || '-'}</td>
                                            <td>{l.location}</td>
                                            <td>{l.updated_at ? moment(l.updated_at).format("DD/MM/YYYY HH:mm") : '-'}</td>
                                            <td>
                                                <button className="btn btn-square btn-ghost" onClick={() => detailCurrentExam(l.id)}>
                                                    <EyeIcon className="w-5"/>
                                                </button>
                                                <button className="btn btn-square btn-ghost" onClick={() => editCurrentData(l.id)}>
                                                    <PencilIcon className="w-5"/>
                                                </button>
                                                <button className="btn btn-square btn-ghost" onClick={() => deleteCurrentData(l.id)}>
