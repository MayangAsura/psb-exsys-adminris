...
function UniformModels(){
    ...
    const [originalUniformModels, setOriginalUniformModels] = useState([]) // Store original data
    const dispatch = useDispatch()
    const navigate = useNavigate()
    ...
    const getUniformModelsData = async () => {
        ...
        if(!error){
            ...
            setOriginalUniformModels(UniformModels) // Store original data
        }
    }

    const removeFilter = () => {
        setUniformModels(originalUniformModels) // Restore original data
    }

    const applyFilter = (params) => {
        const genderValue = params === "Laki-Laki" ? "male" : "female"
        let filteredTransactions = originalUniformModels.filter((t) => {return t.model_gender === genderValue})
    ...
    const applySearch = (value) => {
        if(value === "" || value == null) {
            ...
            setUniformModels(originalUniformModels)
        } else {
            // Filter data based on search value
            let filteredTransactions = originalUniformModels.filter((t) => {
                return t.model_name.toLowerCase().includes(value.toLowerCase()) || 
                       t.model_code.toLowerCase().includes(value.toLowerCase()) ||
                       t.schools?.school_name.toLowerCase().includes(value.toLowerCase())
            })
            setUniformModels(filteredTransactions)
        }
    }
    ...
    const deleteCurrentData = async (index) => {
        dispatch(openModal({
            ...
            extraObject : { 
                message : `Apakah Anda yakin menghapus seragam ini?`, 
