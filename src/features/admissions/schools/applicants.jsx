...
const TopSideButtons = ({applicants, removeFilter, applyFilter, applySearch}) => {
    ...
    const clean_data = applicants.map(value => ({
        ...
        STATUS_PENGISIAN_FORMULIR:	value.participant.is_draft==true?'Lengkap':'Belum Lengkap',
        STATUS_PENGISIAN_UKURAN_SERAGAM: value.participant.is_uniform_sizing==true?'Lengkap':'Belum Lengkap',
        ALAMAT: value.participant.home_address,
        TEMPAT_LAHIR: value.participant.pob,
        TANGGAL_LAHIR: value.participant.dob,
        KATEGORI_SISWA: getParticipantCategory(value.participant.student_category),
        METODE_PEMBAYARAN_UANGPANGKAL: getPilihanMetodeUangPangkalText(value.participant.metode_uang_pangkal),
        CITA_CITA: value.participant.aspiration,
...
//   };
    ...
    return(
        <div className="inline-block float-right z-200" z-index='200'>
            <div className="inline-block float-right">
                <button className="btn px-6 btn-sm normal-case bg-green-700 text-gray-100 hover:bg-green-800 dark:text-gray-600" onClick={() => addNewapplicant()}>Tambah Peserta</button>
            </div>
            <div className="inline-block float-right">
                <button className="btn px-6 btn-sm normal-case bg-orange-700 text-gray-100 hover:bg-orange-800 dark:text-gray-600" onClick={() => exportapplicants()}>Export Peserta</button>
            </div>
    ...
    //     <div className="inline-block float-right">
    //         <div className="inline-block float-right">
    //             <button className="btn px-6 btn-sm normal-case bg-green-700 text-gray-100 hover:bg-green-500 dark:text-gray-600" onClick={() => addNewapplicant()}>Tambah Peserta</button>
    //         {/* <div className="inline-block float-right"> */}
  ...
  const getStatus = (status) => {
    ...
    return(
        <>
            ...
            <TitleCard title="Peserta" topMargin="mt-2" TopSideButtons={<TopSideButtons applicants={Examapplicants} applySearch={applySearch} applyFilter={applyFilter} removeFilter={removeFilter}/>}>
            {/* UJIAN */}
