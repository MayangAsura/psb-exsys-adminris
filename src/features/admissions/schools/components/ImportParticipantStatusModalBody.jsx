...
function ImportParticipantStatusModalBody({ closeModal, extraObject }) {
...
//   });
      ...
      if(data.error===false){
        dispatch(
            openModal({
              ...
              extraObject: {
                message:
                  participants.length + " total Data status Peserta berhasil diupload",
  ...
  const saveImport = (data) => {
    ...
    if (!data || data.length == 0) {
      dispatch(
        showNotification({
          message: "Gagal Import Data Status Peserta. Data tidak valid.",
          status: 0,
        })
      );
      closeModal();
  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center p-2 mt-5">
        <FileUploads save={saveImport} ></FileUploads>
      </div>
      {/* 
...
export default ImportParticipantStatusModalBody;
