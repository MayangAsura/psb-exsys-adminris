...
function EditAdmissionSchoolModalBody({closeModal, extraObject}){
    ...
    useEffect(() => {
        getSchoolsOptions()
        getAdmissionSchool(sch_id)
        if(index){
            setAdmissionSchool({...admissionSchool, admission_id: index})
        }
    }, [index, sch_id])

    const getAdmissionSchool = async (sch_id) => {
        if (sch_id) {
            const {data: admission_schools, error } = await supabase.from('admission_schools')
                                                        .select("*")
                                                        .eq('school_id', sch_id)
                                                        .eq('admission_id', index)
                                                        .is('deleted_at', null)
    ...
    return(
        <>
            ...
            <SelectBox 
                            ...
                            // labelStyle="hidden"
                            defaultValue={admissionSchool.school_id}
...
export default EditAdmissionSchoolModalBody